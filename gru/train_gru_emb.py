...
from dataset_emb import LanguageDataset
from model_emb import GRU
...
def train(config):
    ...
    train_dataset = LanguageDataset(config.train, config.seq_length)
    test_dataset = LanguageDataset(config.test, config.seq_length, train_dataset)
    data_loader_train = DataLoader(train_dataset, config.batch_size, num_workers=0)
    data_loader_test = DataLoader(test_dataset, config.batch_size, num_workers=0)
    # Initialize the model that we are going to use
    model = GRU(config.batch_size, config.embedding_size, train_dataset.vocab_size, train_dataset.n_langs, \
    ...
    for epoch in range(epochs):
        for step, (batch_inputs, batch_targets) in enumerate(data_loader_train):
            ...
            # initialize one hot
            y_pred_batch, _ = model(batch_inputs.type(dtype))
            ...
            if step % config.learning_rate_step is 0 and step is not 0:
                ...
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr
            if step % config.save_every == 0 and step is not 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gru_num_layers', type=int, default=1, help='Number of GRU layers in the model')
